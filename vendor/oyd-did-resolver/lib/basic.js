 ...
 */
import { checkRepositoryUrl } from "./security.js";
...
export function multiEncode(message, options) {
...
export async function multiHash(message, options) {
    ...
    if (encoded[0] === null)
        return [null, encoded[1]];
    return [encoded[0], ""];
}
/** ⇔ hash (basic.rb:32) */
export async function hashDefault(message) {
...
export function getDigest(message) {
    ...
    if (decoded === null)
        return [null, error];
    if (decoded[0] === 0x02 && decoded[1] === 0x10)
        return ["blake2b-16", ""];
    if (decoded[0] === 0x04 && decoded[1] === 0x20)
        return ["blake2b-32", ""];
    if (decoded[0] === 0x08 && decoded[1] === 0x40)
        return ["blake2b-64", ""];
...
export function getEncoding(message) {
    if (message.startsWith("z"))
        return ["base58btc", ""];
    return [null, "unsupported multibase prefix"];
}
...
export function canonical(message) {
...
export function percentEncode(did) {
    return did
        .replace("https://", "")
        .replace("@", "%40")
...
export async function verify(message, signature, publicKey) {
    try {
        ...
        if (digest === null)
            return [null, "unsupported key codec"];
        const [sig] = multiDecode(signature);
        if (sig === null || sig.length !== 64)
            return [false, ""];
        const key = await crypto.subtle.importKey("raw", digest, { name: "Ed25519" }, false, ["verify"]);
        const ok = await crypto.subtle.verify("Ed25519", key, sig, utf8(message));
        return [ok, ""];
    }
    catch {
...
export function getDelegatedPubKeysFromFullDidDocument(didDocument, keyType = "doc") {
    const keyField = didDocument.doc?.key;
    const keys = keyField == null
        ? null
        : keyType === "doc"
            ? [keyField.split(":")[0]]
            : [keyField.split(":")[1]];
    if (keys === null)
        return [null, "cannot retrieve current key"];
...
export function isPubKeyIdentifier(didHash) {
...
async function verifyDocCommitment(identifier, doc) {
    ...
    if (digest[0] === null)
        return [null, digest[1]];
    if (encoding[0] === null)
        return [null, encoding[1]];
    const computed = await multiHash(canonical(doc), {
        digest: digest[0],
        encode: encoding[0],
    });
    if (computed[0] === null)
        return [null, computed[1]];
...
export async function retrieveDocument(docIdentifier, docLocation, options) {
    ...
    const optionQuery = options.followAlsoKnownAs
        ...
        : "";
    const [body, message] = await fetchJson(location + "/doc/" + docIdentifier + optionQuery, options);
    if (body === null)
        return [null, message];
...
export async function retrieveDocumentRaw(docHash, docLocation, options) {
    const hash = stripLocation(docHash).replace(/^did:oyd:/, "");
    const location = normalizeLocation(docLocation);
    const [body, message] = await fetchJson(location + "/doc_raw/" + hash, options);
    if (body === null)
        return [null, message];
    const shell = typeof body === "object" && body !== null && !Array.isArray(body)
        ? body
        : null;
    if (shell === null)
    ...
    if (committed[0] === null)
        return [null, committed[1]];
    return [{ doc, log }, ""];
}
/** ⇔ retrieve_log (log.rb:26, HTTP branch) · spec §4.2.4 #retrieve_log */
export async function retrieveLog(didHash, logLocation, options) {
    const location = normalizeLocation(logLocation);
    const [body, message] = await fetchJson(location + "/log/" + didHash, options);
    if (body === null)
        return [null, message];
