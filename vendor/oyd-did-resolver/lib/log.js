 ...
 */
import { canonical, getDigest, getEncoding, multiHash, retrieveDocumentRaw, retrieveLog, stripLocation, verify, DidError, LOG_HASH_OPTIONS, Op, } from "./basic.js";
...
export class Dag {
    vertices = [];
    addVertex(id) {
        const vertex = { id, successors: [], predecessors: [] };
...
function afterLast(value, separator) {
...
function logSlice(entry) {
    const slice = {};
    for (const field of ["ts", "op", "doc", "sig", "previous"]) {
        if (field in entry)
            slice[field] = entry[field];
    }
    return slice;
}
...
export async function dedupeLogEntries(logs) {
    const seen = new Set();
    const unique = [];
    for (const el of logs) {
        const hash = (await multiHash(canonical(logSlice(el)), LOG_HASH_OPTIONS))[0];
...
export async function matchLogDid(log, doc) {
...
export async function dagDid(logs, options) {
    ...
    const boundOr = (value, fallback) => typeof value === "number" && Number.isInteger(value) && value >= 0
        ? value
        : fallback;
    const maxLogEntries = boundOr(options.maxLogEntries, MAX_LOG_ENTRIES);
    const maxPreviousRefs = boundOr(options.maxPreviousRefs, MAX_PREVIOUS_PER_ENTRY);
    ...
    for (let i = 0; i < logs.length; i++) {
        ...
        logHash.push((await multiHash(canonical(logSlice(el)), LOG_HASH_OPTIONS))[0] ?? "");
...
function firstIndexByHash(logHash) {
    const index = new Map();
    for (let i = 0; i < logHash.length; i++) {
        if (!index.has(logHash[i]))
            index.set(logHash[i], i);
    }
    return index;
}
...
export function dag2array(dag, logArray, index, result, visited = new Set()) {
...
export function dag2arrayTerminate(dag, logArray, index, result) {
    const vertex = dag.vertices[index];
    if (vertex) {
...
export async function dagUpdate(currentDID, options) {
    let i = 0;
    let revoked = false;
    ...
    walk: for (const el of currentDID.log) {
        switch (el.op) {
            ...
            case Op.TERMINATE: {
                ...
                if ((await multiHash(canonical(logSlice(el)), logOptions))[0] !== term) {
                    currentDID.error = DidError.INVALID;
                ...
                const [rawRevocationLog] = await retrieveLog(didHash, logLocation, options);
                ...
                for (const logEl of logArray) {
                    const structure = { ...logEl };
                    if (logEl.op === Op.REVOKE) {
                        // REVOKE records are hashed without their `previous` attribute
                        delete structure["previous"];
                    }
                    if ((await multiHash(canonical(logSlice(structure)), logOptions))[0] ===
                        revocTerm) {
            ...
            case Op.REVOKE: {
                ...
                if (i === currentDID.log.length - 1 && options.followAlsoKnownAs) {
                    ...
                    const rotateDID = typeof payload === "object" &&
                        payload !== null &&
                        !Array.isArray(payload)
                        ? String(payload["alsoKnownAs"] ?? "")
                        : "";
