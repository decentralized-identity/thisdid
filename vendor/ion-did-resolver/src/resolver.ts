...
function multihash(canonical: string): string {
...
interface PatchDocument {
  publicKeys?: SidetreePublicKey[];
  services?: SidetreeService[];
}
...
interface InitialState {
  suffixData?: { deltaHash?: string; recoveryCommitment?: string };
  delta?: { patches?: Patch[]; updateCommitment?: string };
}
...
function applyPatches(patches: Patch[]): {
  ...
  let publicKeys: SidetreePublicKey[] = [];
  let services: SidetreeService[] = [];
    ...
    switch (patch.action) {
      ...
      default:
        throw new ResolutionError(
          "invalidDidDocument",
          `unsupported patch action \`${String(patch.action)}\``,
        );
    }
  }
...
function composeDocument(
  ...
  for (const key of publicKeys) {
    if (!key.id || !ID_FRAGMENT_RE.test(key.id) || seen.has(key.id)) {
      throw new ResolutionError(
        "invalidDidDocument",
        "public key ids must be unique 1-50 char base64url fragments",
      );
    }
    seen.add(key.id);
  ...
  return {
    id: did,
    "@context": ["https://www.w3.org/ns/did/v1", { "@base": did }] as never,
...
function resolveLongForm(
  ...
  try {
    state = JSON.parse(new TextDecoder().decode(bytes)) as InitialState;
  } catch {
  ...
  if (!suffixData || !delta) {
    throw new ResolutionError(
      "invalidDid",
      "initial state must carry suffixData and delta",
    );
  }
  ...
  if (base64urlEncode(new TextEncoder().encode(jcs(state))) !== encodedState) {
    throw new ResolutionError(
      "invalidDid",
      "initial state is not canonically encoded",
    );
  }
  ...
  if (multihash(jcs(suffixData)) !== suffix) {
    throw new ResolutionError(
      "invalidDid",
      "DID suffix does not match its initial state",
    );
  }
  ...
  if (canonicalDelta.length > MAX_CANONICAL_DELTA_CHARS) {
    throw new ResolutionError(
      "invalidDidDocument",
      "delta exceeds the Sidetree size bound",
    );
  }
  if (multihash(canonicalDelta) !== suffixData.deltaHash) {
    throw new ResolutionError(
      "invalidDidDocument",
      "deltaHash does not commit to the delta",
    );
  }
  ...
  return {
    ...
    didDocumentMetadata: {
      ...
      equivalentId: [`did:ion:${suffix}`] as unknown as string,
    },
  };
}

/** Sidetree / Universal Resolver error spellings → DIF codes. */
function normalizeError(raw: unknown, status: number): string {
  const flat = String(
    ...
    .replace(/[_\s-]/g, "")
    .toLowerCase();
  if (flat.includes("notfound")) return "notFound";
  if (flat.includes("deactivated")) return "deactivated";
  if (flat.includes("invaliddid") || flat.includes("didstring")) {
    return "invalidDid";
  }
  if (flat) return flat === "internalerror" ? "internalError" : "upstreamError";
  return status === 404 ? "notFound" : "upstreamError";
}

export function getResolver(options?: IonResolverOptions): ResolverRegistry {
  ...
  async function resolveShortForm(
    did: string,
    ...
    if (!base) {
      throw new ResolutionError(
        "notConfigured",
        "no anchored-state endpoint configured; short-form did:ion is delegated to the routing chain",
      );
    }
    const params = new URLSearchParams();
    const extra = resolutionOptions as Record<string, unknown>;
    ...
    const response = await fetch(
      `${base}/identifiers/${encodeURIComponent(did)}${query}`,
      {
        headers: {
          accept:
    ...
    try {
      ...
      throw new ResolutionError(
        response.ok ? "invalidResponse" : "upstreamError",
        `endpoint HTTP ${response.status}`,
      );
    }
    const upstreamError =
      body.didResolutionMetadata?.error ??
    ...
    if (!response.ok || upstreamError || !didDocument?.id) {
      ...
      throw new ResolutionError(
        normalizeError(upstreamError, response.status),
        "",
      );
    }
    ...
    const equivalent = Array.isArray(documentMetadata.equivalentId)
      ? (documentMetadata.equivalentId as string[])
      : [];
    if (
      didDocument.id !== did &&
      canonicalId !== did &&
      ...
      throw new ResolutionError(
        "invalidDidDocument",
        "endpoint returned a document for a different DID",
      );
    }
    return {
      didResolutionMetadata: { contentType: "application/did+ld+json" },
  ...
  const ion = async (
    ...
    try {
      ...
      if (segments[2] === "test") {
        return errorResult(
          "notFound",
          "the ION test network was sunset and is not resolvable",
        );
      }
  ...
  return { ion } as unknown as ResolverRegistry;
}
