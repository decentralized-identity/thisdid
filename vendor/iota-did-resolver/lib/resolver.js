...
async function rpcCall(endpoint, method, params, timeoutMs) {
    try {
        ...
        if (!response.ok)
            return { error: `IOTA RPC HTTP ${response.status}` };
        const text = await readBounded(response);
        if (text === null)
            return { error: "response exceeds size bound" };
        let parsed;
        try {
        ...
        return {
            error: cause instanceof Error ? cause.message.slice(0, 200) : "fetch failed",
        };
    }
}
/**
...
async function assertChain(endpoint, expected, timeoutMs) {
    let actual = chainIdCache.get(endpoint);
    if (!actual) {
        const answer = await rpcCall(endpoint, "iota_getChainIdentifier", [], timeoutMs);
...
export function resetChainIdCache() {
    chainIdCache.clear();
}
/**
 ...
 */
export function unpackDidDocument(bytes) {
    if (bytes.length < 7)
        return "document bytes too short";
    if (bytes[0] !== 0x44 || bytes[1] !== 0x49 || bytes[2] !== 0x44) {
        return "missing DID magic bytes";
    }
    if (bytes[3] !== 1)
        return `unsupported document version ${bytes[3]}`;
    if (bytes[4] !== 0)
        return `unsupported document encoding ${bytes[4]}`;
    ...
    const metaObject = meta && typeof meta === "object" && !Array.isArray(meta)
        ? meta
        : {};
    return { doc: doc, meta: metaObject };
}
/** Replace every `did:0:0` placeholder inside string values with the DID. */
function substitutePlaceholder(value, did) {
    ...
    const iota = async (did) => {
        try {
            ...
            if (segments[0] !== "did" ||
                ...
                segments.length < 3 ||
                segments.length > 4) {
                return errorResult("invalidDid", "expected did:iota:(<network>:)?0x<64-hex-object-id>");
            }
            const tag = segments[segments.length - 1].toLowerCase();
            if (!OBJECT_ID_RE.test(tag)) {
                return errorResult("invalidDid", "tag must be a 0x-prefixed 32-byte hex object id");
            }
            ...
            if (!endpoints.length) {
                return errorResult("notConfigured", `no IOTA fullnode configured for network \`${network}\``);
            }
            ...
            const expectedChain = CHAIN_IDS[network] ?? (CHAIN_ID_RE.test(network) ? network : null);
            const canonical = network === "iota" ? `did:iota:${tag}` : `did:iota:${network}:${tag}`;
            ...
            for (const endpoint of endpoints) {
                if (expectedChain) {
                    const mismatch = await assertChain(endpoint, expectedChain, timeoutMs);
                    if (mismatch) {
                        lastTransportError = mismatch;
                        continue;
                    }
                }
                const call = await rpcCall(endpoint, "iota_getObject", [tag, { showContent: true, showType: true }], timeoutMs);
            ...
            if (answer.error) {
                if (answer.error.code === "notExists") {
                    return errorResult("notFound", "no object with this id on the network");
                }
                if (answer.error.code === "deleted") {
                    return errorResult("notFound", "the object has been deleted");
                }
                return errorResult("networkError", `IOTA object error ${answer.error.code ?? "unknown"}`);
            }
            const data = answer.data;
            if (!data?.content?.fields) {
                return errorResult("networkError", "IOTA RPC answer has no object content");
            }
            const typeMatch = IDENTITY_TYPE_RE.exec(data.type ?? data.content.type ?? "");
            if (!typeMatch) {
                return errorResult("invalidDidDocument", "object is not an `identity::Identity` Move object");
            }
            const knownPackages = IDENTITY_PACKAGES[network];
            if (knownPackages && !knownPackages.includes(typeMatch[1])) {
                return errorResult("invalidDidDocument", "object type is not from the network's published identity package");
            }
            ...
            const didDoc = fields.did_doc;
            const raw = didDoc?.fields?.controlled_value;
            if (!Array.isArray(raw) || !raw.every((b) => typeof b === "number")) {
                return errorResult("invalidDidDocument", "Identity holds no DID document bytes");
            }
            ...
            const substituted = substitutePlaceholder(unpacked.doc, canonical);
            if (substituted.id !== canonical) {
                return errorResult("invalidDidDocument", "stored document id is not the method placeholder");
            }
            const document = {
