...
function isOwnFragmentId(value, did) {
 ...
 */
function isValidEventEntry(kind, entry, did) {
    if (kind === "DIDOwner") {
        return (entry.controller === undefined || typeof entry.controller === "string");
    }
    if (!isOwnFragmentId(entry.id, did))
        return false;
    if (kind === "Service") {
    ...
    const hasMaterial = typeof entry.publicKeyBase58 === "string" ||
        typeof entry.publicKeyMultibase === "string";
    const validRelationship = kind !== "VerificationRelationship" ||
...
export function getResolver(options) {
     ...
     */
    async function fetchTopicMessages(mirrorBase, topicId) {
        const messages = [];
        let path = `/api/v1/topics/${topicId}/messages?limit=${PAGE_LIMIT}&order=asc`;
        const pageCap = Math.ceil(maxMessages / PAGE_LIMIT) + 2;
        const truncated = () => new ResolutionError("resourceLimitExceeded", `topic has more than ${maxMessages} messages; refusing to resolve from partial history`);
        for (let page = 0;; page++) {
            if (page >= pageCap)
                throw truncated();
            ...
            if (!response.ok) {
                throw new ResolutionError("networkError", `mirror node HTTP ${response.status}`);
            }
    ...
    function applyMessage(state, did, rootKey, raw) {
        ...
        try {
            if (!ed25519.verify(signature, payload, rootKey))
                return;
        }
        catch {
            return;
        ...
        let event;
        try {
            event = JSON.parse(new TextDecoder().decode(eventBytes));
        }
        catch {
            return;
        }
        const apply = (kind, entry) => {
            const id = String(entry.id ?? "");
            if (operation === "revoke") {
                if (kind === "VerificationMethod")
                    state.methods.delete(id);
            ...
            if (!isValidEventEntry(kind, entry, did))
                return;
            if (kind === "DIDOwner") {
                state.owner = entry;
    ...
    const hedera = async (did) => {
        try {
            ...
            if (rest.length > 0 ||
                ...
                return errorResult("invalidDid", "expected did:hedera:<network>:z<base58-key>_<shard.realm.num>");
            }
            const rootKey = base58Decode(idString.slice(1));
            if (rootKey?.length !== 32) {
                return errorResult("invalidDid", "identifier is not a base58 Ed25519 public key");
            }
            const mirrorBase = mirrorUrls[network]?.replace(/\/+$/, "");
            if (!mirrorBase) {
                return errorResult("notConfigured", `no mirror node configured for network \`${network}\``);
            }
            ...
            for (const raw of rawMessages)
                applyMessage(state, did, rootKey, raw);
            ...
            if (!state.owner) {
                return errorResult("notFound", "no validly signed DIDOwner event on the topic");
            }
            ...
            for (const entry of state.relationships.values()) {
                ...
                if (!RELATIONSHIP_TYPES.has(type) || !id)
                    continue;
            ...
            const service = [...state.services.values()];
            const didDocument = {
                "@context": CONTEXTS,
                id: did,
                controller: state.owner.controller ?? did,
