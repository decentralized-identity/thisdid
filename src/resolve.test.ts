...
describe('resolveDid', () => {
  it('rejects malformed DIDs before routing', async () => {
    expect((await resolveDid('not-a-did', env)).didResolutionMetadata.error).toBe('invalidDid')
  })

  it('returns unsupportedDidMethod for methods outside the advertised catalog', async () => {
    expect((await resolveDid('did:unknown:123', env)).didResolutionMetadata.error).toBe('unsupportedDidMethod')
  })

  it('preserves meaningful upstream errors and attempt diagnostics', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => Response.json({
      didResolutionMetadata: { error: 'deactivated', upstreamField: 'retained' },
      didDocument: null,
      didDocumentMetadata: { deactivated: true },
    }, { status: 410 })))
    const result = await resolveDid('did:algo:abc', env)
    expect(result.didResolutionMetadata.error).toBe('deactivated')
    expect(result.didResolutionMetadata.attempted).toEqual(['goplausible', 'godiddy', 'archon'])
    expect(result.didDocumentMetadata).toEqual({ deactivated: true })
  })

  it('rejects an upstream document for a different DID', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => Response.json({
      didResolutionMetadata: {}, didDocument: { id: 'did:algo:different' }, didDocumentMetadata: {},
    })))
    const result = await resolveDid('did:algo:requested', env)
    expect(result.didDocument).toBeNull()
    expect(result.didResolutionMetadata.attempts?.every((a) => a.error === 'invalidDidDocument')).toBe(true)
  })
})
...
it('moves down providers behind healthy fallbacks without deleting them', () => {
  const health: HealthSnapshot = {
    v: 1,
    updatedTs: Date.now(),
    providers: { local: { status: 'down', ewmaMs: null, successRate: 0, consecutiveFails: 3, lastOkTs: null, lastProbeTs: Date.now() } },
  }
  expect(planChain(['local', 'godiddy', 'archon'], health)).toEqual(['godiddy', 'archon', 'local'])
})
