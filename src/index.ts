...
function wantsJson(accept: string | undefined): boolean {
  ...
  return (
    /application\/(did\+)?(ld\+)?json/i.test(accept) ||
...
async function limited(
  ...
  return success
    ...
    : c.json(
        {
          didResolutionMetadata: { error: "rateLimitExceeded" },
          didDocument: null,
          didDocumentMetadata: {},
        },
        429,
...
app.post("/mcp", async (c) => {
  ...
  const response = await handleMcp(request, c.env);
  return response ? c.json(response) : c.body(null, 202);
});
app.get("/mcp", (c) =>
  c.json(
    { error: "methodNotAllowed", message: "Use POST with MCP JSON-RPC." },
    405,
    { Allow: "POST" },
  ),
);
...
app.get("/health", (c) =>
  c.json({ status: "ok", service: c.env.RESOLVER_LABEL }),
);
...
app.get("/data", async (c) =>
  c.json(await getStats(c.env, parseFilter(new URL(c.req.url)))),
);
...
app.get("/analytics", async (c) => {
  if (wantsJson(c.req.header("accept")))
    return c.json(await getStats(c.env, parseFilter(new URL(c.req.url))));
