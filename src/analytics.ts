 ...
 *  - KV (`env.STATS_KV`) → a live lifetime counter (`analytics:total`, best-effort)
 *                         plus a read-through cache of the unfiltered summaries.
 *
...
async function ensureSchema(db: D1Database): Promise<void> {
  ...
  const info = await db.prepare('PRAGMA table_info(resolutions)').all<{ name: string }>()
  ...
  for (const [name, type] of columns) {
    if (!have.has(name)) {
      try {
        await db.prepare(`ALTER TABLE resolutions ADD COLUMN ${name} ${type}`).run()
      } catch {
  ...
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_resolutions_ts ON resolutions (ts)').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_resolutions_method ON resolutions (method)').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_resolutions_provider ON resolutions (provider)').run()
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_resolutions_country ON resolutions (country)').run()
  schemaReady = true
}

export async function recordResolution(env: Env, ev: ResolutionEvent): Promise<void> {
  try {
    if (env.DB) {
...
const RANGES: Record<TimeRange, { window: number | null | 'ytd'; bucket: Bucket }> = {
...
function whereClause(f: StatsFilter): { sql: string; binds: (string | number)[] } {
...
function emptyStats(filter: StatsFilter): Stats {
  return {
    ...
    totals: { total: 0, liveTotal: 0, success: 0, errors: 0, successRate: 0, latencyTotalMs: 0, latencyAvgMs: 0 },
    byMethod: [],
    byProvider: [],
    byResolver: [],
    byCountry: [],
 ...
 */
export async function recentPage(env: Env, filter: StatsFilter, before?: string, limit = RECENT_LIMIT): Promise<RecentPage> {
  const db = env.DB
  ...
  const recent: RecentRow[] = results.map((r) => ({ ...r, success: !!r.success }))
  const last = recent[recent.length - 1]
  const nextCursor = recent.length === n && last ? `${last.ts}_${last.id}` : null
...
async function computeStats(env: Env, filter: StatsFilter): Promise<Stats> {
  ...
  const [totalsR, latR, tlR, recentR, calR, cOptR, mOptR, mR, pR, rR, cR] = await db.batch<Record<string, number | string | null>>([
    db.prepare(`SELECT COUNT(*) total, SUM(success) ok, SUM(duration_ms) lat, AVG(duration_ms) avg_ms FROM resolutions ${w.sql}`).bind(...w.binds),
    db
      .prepare(`SELECT COALESCE(provider,'—') k, COUNT(*) c, AVG(duration_ms) avg, MIN(duration_ms) lo, MAX(duration_ms) hi FROM resolutions ${w.sql} ${and} success = 1 AND provider IS NOT NULL GROUP BY k ORDER BY c DESC`)
      .bind(...w.binds),
    db.prepare(`SELECT ${bucketExpr} t, COUNT(*) c, SUM(success) ok FROM resolutions ${w.sql} GROUP BY t ORDER BY t`).bind(...w.binds),
    db.prepare(`SELECT ${RECENT_COLS} FROM resolutions ${w.sql} ORDER BY ts DESC, id DESC LIMIT ?`).bind(...w.binds, RECENT_LIMIT),
    db.prepare(`SELECT date(ts/1000,'unixepoch') d, COUNT(*) c FROM resolutions WHERE ${calClauses.join(' AND ')} GROUP BY d`).bind(...calBinds),
  ...
  const t = (totalsR.results[0] ?? {}) as { total: number; ok: number | null; lat: number | null; avg_ms: number | null }
  const total = t.total ?? 0
  const success = t.ok ?? 0
  const toCounts = (r: typeof mR) => (r.results as { k: string; c: number }[]).map((x) => ({ key: x.k, count: x.c }))
  const recentRows = recentR.results as (Omit<RecentRow, 'success'> & { success: number })[]
  const recent: RecentRow[] = recentRows.map((r) => ({ ...r, success: !!r.success }))
  const last = recent[recent.length - 1]
  ...
  return {
    ...
    latencyByProvider: (latR.results as { k: string; c: number; avg: number; lo: number; hi: number }[]).map((r) => ({ key: r.k, count: r.c, avgMs: Math.round(r.avg), minMs: r.lo, maxMs: r.hi })),
    timeline: { granularity, points: (tlR.results as { t: string; c: number; ok: number }[]).map((r) => ({ t: r.t, count: r.c, success: r.ok, errors: r.c - r.ok })) },
    calendar: (calR.results as { d: string; c: number }[]).map((r) => ({ day: r.d, count: r.c })),
    recent,
    recentCursor: recent.length === RECENT_LIMIT && last ? `${last.ts}_${last.id}` : null,
    configured: true,
  }
}
...
export async function getStats(env: Env, filter: StatsFilter): Promise<Stats> {
  ...
  if (cacheable && env.STATS_KV) {
    ...
    env.STATS_KV.put(key, JSON.stringify(stats), { expirationTtl: SUMMARY_TTL }).catch(() => {})
  }
  return stats
}
