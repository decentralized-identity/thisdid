...
export function openApiSpec(origin: string) {
  return {
    ...
    servers: [{ url: origin }],
    tags: [
      { name: 'Resolution', description: 'DIF Universal Resolver HTTP binding' },
      { name: 'Discovery', description: 'Supported methods & service metadata' },
    ...
    paths: {
        ...
        get: {
          ...
          parameters: [
            {
              ...
              in: 'path',
              required: true,
              description: 'The DID to resolve, e.g. `did:web:identity.foundation`.',
              schema: { type: 'string' },
          ...
          responses: {
            '200': {
              ...
              content: {
                'application/ld+json': { schema: { $ref: '#/components/schemas/DIDResolutionResult' } },
                'application/json': { schema: { $ref: '#/components/schemas/DIDResolutionResult' } },
              },
            },
            '400': { description: 'Invalid DID syntax (`error: invalidDid`).' },
            '404': { description: 'DID not found (`error: notFound`).' },
            '501': { description: 'Unsupported DID method (`error: unsupportedDidMethod`).' },
          },
        },
      },
      '/methods': {
              ...
              content: {
                'application/json': {
                  schema: {
                    type: 'object',
                    properties: {
                      featured: { type: 'array', items: { $ref: '#/components/schemas/MethodMeta' } },
      ...
      '/recent': {
        get: {
          ...
          parameters: [
            { name: 'before', in: 'query', description: 'Cursor from a previous `nextCursor`.', schema: { type: 'string' } },
          ...
          responses: {
            '200': {
              ...
              content: { 'application/json': { schema: { type: 'object', properties: { recent: { type: 'array' }, nextCursor: { type: ['string', 'null'] } } } } },
            },
          },
        },
      },
      '/mcp': {
        post: {
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: { type: 'object', description: 'JSON-RPC 2.0 MCP request envelope.' },
                example: { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'resolve_did', arguments: { did: 'did:web:identity.foundation' } } },
              },
            },
          },
          responses: {
          ...
          properties: {
            ...
            didResolutionMetadata: {
              ...
              properties: {
                ...
                via: { type: 'string' },
                chain: { type: 'string', description: 'Ordered fallback chain attempted, e.g. local→godiddy→archon' },
              },
            },
