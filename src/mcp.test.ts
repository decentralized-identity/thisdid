...
describe("MCP endpoint handler", () => {
    ...
    const response = await handleMcp(
      {
        ...
        id: 3,
        method: "tools/call",
        params: { name: "describe_routing", arguments: { method: "did:algo" } },
      },
      env,
    );
    const text = (response?.result as { content: Array<{ text: string }> })
      .content[0].text;
    expect(JSON.parse(text).chain).toEqual([
      "goplausible",
      "godiddy",
      "archon",
    ]);
  ...
  it("rejects null, arrays, and invalid parameter shapes without throwing", async () => {
