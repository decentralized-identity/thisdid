import type { DIDDocument, DIDResolutionResult, VerificationMethod } from './types'
...
const refId = (x: string | VerificationMethod): string => (typeof x === 'string' ? x : x.id)
...
function buildView(did: string, resolution: DIDResolutionResult, doc: DIDDocument): ResultView {
  ...
  const usesFor = (id: string): string[] =>
    REL_DEFS.filter((r) => ((doc[r.key] as (string | VerificationMethod)[]) ?? []).some((x) => refId(x) === id)).map(
      (r) => r.key as string,
    )
  ...
  const vmList: VmCard[] = vms.map((vm) => {
    ...
    if (vm.publicKeyMultibase) {
      ...
      keyValue = `${vm.publicKeyJwk.crv ?? vm.publicKeyJwk.kty ?? 'JWK'} · ${vm.publicKeyJwk.x ?? ''}`.trim()
      keyLabel = 'publicKeyJwk'
    } else if (vm.blockchainAccountId) {
      keyValue = vm.blockchainAccountId
      keyLabel = 'blockchainAccountId'
    }
    return { frag: frag(vm.id), type: vm.type, keyLabel, keyValue, uses: usesFor(vm.id) }
  })

  const relList: RelRow[] = REL_DEFS.filter((r) => (doc[r.key] as unknown[])?.length).map((r) => ({
    name: r.key as string,
    accent: r.accent,
    refs: (doc[r.key] as (string | VerificationMethod)[]).map((x) => frag(refId(x))).join(', '),
  }))
  ...
  const svcList: SvcRow[] = svc.map((s) => ({
    ...
    endpoint: endpointStr(s.serviceEndpoint),
  }))

  const controller = Array.isArray(doc.controller) ? doc.controller[0] : (doc.controller ?? doc.id)
...
export async function resolveDid(input: string): Promise<ResolveOutcome> {
  const did = input.trim()
  const res = await fetch(`${API_BASE}/1.0/identifiers/${encodeURIComponent(did)}`, {
